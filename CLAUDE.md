...
`/flow` is a state machine implemented as an orchestrator skill that dispatches to bounded subagents and never writes code itself. Each phase routes work to the cheapest subagent that can do it: Haiku for classification/command-execution/templating (triager, test-runner, reporter), Sonnet for moderate judgment (PM, test-author, failure-triager, security-reviewer), Opus for the load-bearing reasoning calls (architect, planner, implementer). Subagents run in fresh contexts and die after returning. Cross-phase state is split: the orchestrator holds runtime counters and dispatch decisions in its working memory, while artifacts (TASK / ARCHITECT / PLAN / SECURITY / REPORT) and persistent counters/triage live on disk in a per-task directory at `~/.claude/tasks/{project_folder}/{unix_ts}/`. Subagents read what they need from that directory rather than receiving full artifacts in-band.
